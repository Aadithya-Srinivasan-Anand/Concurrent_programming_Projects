/**
	Program to perform Time_Count testing for high contention on the skip list.
...
void high_contention_Time_Count_thread(){
...
void high_contention_Time_Count(){   
    skiplist = Skip_List(3, 0.3);
    ...
    for(size_t i = 0; i < num_threads; i++){
        threads.push_back(thread(high_contention_Time_Count_thread));
    }
    for (auto &th : threads) {
        th.join();
    }
